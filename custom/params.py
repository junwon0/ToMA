...
def add_custom_args(parser):
    ...
    parser.add_argument(
        "--topk", type=int, default=4, help="Topk for sim."
    )
    ...
    parser.add_argument(
        "--topo-reg-stage1",
        default=None,
        help="Topology regulization of image-text.",
    )
    parser.add_argument(
        "--topo-reg-stage2",
        default=None,
    ...
    parser.add_argument(
        "--topo-reg-type",
        default="ToMA",
        help="Topology regulization type.",
    )
    parser.add_argument(
        "--topo-reg-scale",
        type=float,
        default=0.5,
        help="Topology regulization loss scale.",
    )
    parser.add_argument(
        "--topo-reg-1dim",
        type=float,
        default=1.,
        help="1 dimensional Topology regulization loss scale w.r.t 0 dimension",
    )
    parser.add_argument(
        "--sampling-data-domain",
        default=False,
        help="Whether sample data from most highest ratio.",
    )
    ###############################################################################
    return parser
...
def parse_args(args):
    ...
    if args.train_data is None and args.name is not None:
        if args.stage == 1:
            args.resume = f"results/{args.name}/checkpoints/epoch_25.pt"
        elif args.stage == 2:
            args.resume = f"results/{args.name}/checkpoints/epoch_15.pt"
